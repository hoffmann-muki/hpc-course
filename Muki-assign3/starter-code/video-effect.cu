...
__global__ void convolveGPU(float const* in, float *out, int width, int height, float const* kernel, int kernelWidth, int kernelHeight) {
    ...
    for (int i = startY; i < height; i += strideY) {
        for (int j = startX; j < width; j += strideX) {
            ...
            const int outIdx = i * width * 3 + j * 3;

            if (i < halfKernelHeight || i >= height - halfKernelHeight ||
                j < halfKernelWidth || j >= width - halfKernelWidth) {
                out[outIdx + blueChannel] = 0.0f;
                out[outIdx + greenChannel] = 0.0f;
                out[outIdx + redChannel] = 0.0f;
                continue;
            }
            ...
            for (int k = -halfKernelHeight; k <= halfKernelHeight; k += 1) {
                for (int l = -halfKernelWidth; l <= halfKernelWidth; l += 1) {
                    const int inIdx = (i + k) * width * 3 + (j + l) * 3;
                    const int kernelIdx = (k + halfKernelHeight) * kernelWidth + (l + halfKernelWidth);
                    const float kernelVal = kernel[kernelIdx];

                    redDot += in[inIdx + redChannel] * kernelVal;
                    greenDot += in[inIdx + greenChannel] * kernelVal;
                    blueDot += in[inIdx + blueChannel] * kernelVal;
                }
            }
