...
int main(int argc, char **argv) {
    ...
    while (video.nextFrames(inputBatch)) {
        ...
        if (dumpFrameEnabled) {
            const int batchStartFrame = batchCounter * batchSize;
            const int batchEndFrameExclusive = batchStartFrame + static_cast<int>(outputBatch.size());
            if (frameIdx >= batchStartFrame && frameIdx < batchEndFrameExclusive) {
                video.dumpFrame(outputBatch, frameIdx - batchStartFrame, frameFilename);
            }
        }
        batchCounter += 1;
    }
